...
static void bench_ct_nopre_alloc(benchmark::State &state)
{
    SerializeBuffer4 buf;
    benchmark::DoNotOptimize(buf);
    while (state.KeepRunning()) {
        buf = SerializeBuffer4();
    }
}

static void bench_ct_nopre(benchmark::State &state)
{
    SerializeBuffer4 buf;
    benchmark::DoNotOptimize(buf.m_buffer);
    while (state.KeepRunning()) {
...
static void bench_ct_pre_alloc(benchmark::State &state)
{
    SerializeBuffer4 buf(50);
    benchmark::DoNotOptimize(buf);
    while (state.KeepRunning()) {
        buf = SerializeBuffer4(50);
    }
}

static void bench_ct_pre(benchmark::State &state)
{
    SerializeBuffer4 buf(50);
    benchmark::DoNotOptimize(buf.m_buffer);
    while (state.KeepRunning()) {
...
BENCHMARK(bench_ct_nopre_alloc);
BENCHMARK(bench_ct_nopre);
...
BENCHMARK(bench_ct_pre_alloc);
BENCHMARK(bench_ct_pre);
